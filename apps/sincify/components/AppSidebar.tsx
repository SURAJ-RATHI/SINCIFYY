...
export function AppSidebar({ isOpen, onClose, canvasColor, setCanvasColor, isMobile, isStandalone, onClearCanvas, onExportCanvas }: SidebarProps) {
    ...
    }, [clearDialogOpen, isOpen, isCreateRoomOpen, onClose])
    ...
    return (
        <>
            ...
            <section data-sidebar className={cn("transition-transform duration-300 ease-in-out z-20", isMobile ? "" : "absolute top-full mt-2")}>
                <div className={cn("flex flex-col", isMobile ? "" : "Island rounded-lg max-h-[calc(100vh-135px)]")}>
                    <div className={cn("py-1", isMobile ? "" : "overflow-auto py-1 custom-scrollbar")}>
                        <nav className={cn("grid gap-1", isMobile ? "px-0" : "px-2")}>
                            ...
                            {isStandalone ? (
                                <>
                                    ...
                                    <SidebarItem icon={Share2} label="Live collaboration" onClick={() => setIsCreateRoomOpen(true)} />
