...
export default function CanvasBoard() {
    ...
    const [engineSessionKey, setEngineSessionKey] = useState("standalone");
    const [participants, setParticipants] = useState<RoomParticipants[]>([]);
    const [isConnected, setIsConnected] = useState(false);
    const [isCanvasReady, setIsCanvasReady] = useState(false);
    const initializedWithSessionKey = useRef<string | null>(null);
    ...
    useEffect(() => {
        ...
        const updateRoomParams = () => {
            ...
            if (currentRoomParams) {
                ...
                const roomId = currentRoomParams.roomId;
                const encryptionKey = currentRoomParams.encryptionKey;
                const userName = `Guest-${guestSessionId.slice(-4)}`;
                setMode("room");
                userRef.current = {
                    roomId,
                    encryptionKey,
                ...
                setEngineSessionKey(`room:${roomId}:${encryptionKey}:${guestId}`);
            } else {
                setMode("standalone");
                setEngineSessionKey("standalone");
            }
        };
    ...
    useEffect(() => {
        ...
        if (initializedWithSessionKey.current !== engineSessionKey) {
            ...
            const waitReaddy = setTimeout(() => {
                ...
                if (engine) {
                    initializedWithSessionKey.current = engineSessionKey;
    ...
    }, [handleKeyDown, initializeCanvasEngine, isCanvasReady, isConnected, engineSessionKey, canvasEngineState.engine]);
    ...
    const exportCanvas = useCallback(() => {
        ...
        link.download = `sincify-${Date.now()}.png`;
        link.click();
    }, []);
    ...
    return (
        <div className={cn("Sincify h-screen overflow-hidden",
            ...
            <div className="App_Menu App_Menu_Top fixed z-[4] top-4 right-4 left-4 flex justify-center items-center xs670:grid xs670:grid-cols-[1fr_auto_1fr] xs670:gap-4 md:gap-8 xs670:items-start">
                    ...
                    <div className="Main_Menu_Stack Sidebar_Trigger_Button xs670:grid xs670:gap-[calc(.25rem*6)] grid-cols-[auto] grid-flow-row grid-rows auto-rows-min justify-self-start">
                        <div className="relative flex items-center gap-2">
                            ...
                            <Button
                                ...
                                variant="outline"
                                onClick={goToHome}
                                className="h-11 w-11 rounded-2xl border-sidebar-border bg-island-bg-color p-0 hover:bg-button-hover-bg"
                ...
                {matches && (
                    <CollaborationToolbar participants={participants} showShare={mode === "room"} />
                )}
            </div>
            ...
            <div className="Sincify-textEditorContainer"></div>
            ...
            <canvas className={cn("Sincify Sincify-canvas touch-none", theme === 'dark' ? 'Sincify-canvas-dark' : '')} ref={canvasRef} />
        </div >
    )
};
