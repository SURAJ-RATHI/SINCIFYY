...
const WS_PORT = Number(process.env.WS_PORT || process.env.PORT) || 8081;
...
wss.on("connection", function connection(ws, req) {
  ...
  const resolvedUserId = `guest_${connectionId}`;
  const newConnection: Connection = {
    connectionId,
    userId: resolvedUserId,
    userName: resolvedUserId.startsWith("guest_")
      ? `Guest-${resolvedUserId.slice(-4)}`
      : resolvedUserId,
    ws,
    rooms: [],
  };
  ...
  ws.on("error", (err) =>
    console.error(`WebSocket error for user ${newConnection.userId}:`, err)
  );
    ...
    if (connection) {
      ...
      connection.rooms.forEach((roomId) => {
        ...
        if (roomIsEmpty) {
          ...
          console.log(`Deleted empty room ${roomId} after last connection left`);
        }
      });
    }
